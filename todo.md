...
- [x] Task 7: Implement object size and shape measurement functions that compute area, perimeter, centroid, bounding box, eccentricity, and form factor for labeled objects in segmented images (object_measurement.py + tests/test_object_measurement.py)
- [x] Task 8: Implement image intensity measurement functions that compute min, max, mean, median, standard deviation, and percentile statistics for pixel intensities within object regions (intensity_measurement.py + tests/test_intensity_measurement.py)
- [x] Task 9: Implement image transformation functions including resize, crop, flip, and rotate operations that modify image dimensions and orientation with configurable interpolation methods (transforms.py + tests/test_transforms.py)
